...
router.get('/refresh',async (req,res)=>{
    ...
    if (cookies?.jwtrefreshtoken){
        ...
        if(refreshfounduser){
            ...
            await jwt.verify(cookies.jwtrefreshtoken,`${process.env.REQUEST_KEY}`, async (err,decoded)=>{
                // let refreshTokenCount=0;
                if(err){ return res.status(403).send(`sent refresh token is not valid with err ${err}`)}
                else{

                    const accessToken= jwt.sign({email:decoded.email},`${process.env.REQUEST_KEY}`,{expiresIn:'30s'})
                    const refreshToken=jwt.sign({email:refreshfounduser.email},`${process.env.REQUEST_KEY}`,{expiresIn:'30s'})
